...
: Node("radar_crossing_objects_noise_filter", node_options)
{
  ...
  node_param_.velocity_threshold = declare_parameter<double>("velocity_threshold", 0.0);
...
bool RadarCrossingObjectsNoiseFilterNode::isNoise(const DetectedObject & object)
{
  ...
  const double object_position_angle_from_base_link = std::atan2(
    object.kinematics.pose_with_covariance.pose.position.y,
    object.kinematics.pose_with_covariance.pose.position.x);
  const double crossing_yaw =
    tier4_autoware_utils::normalizeRadian(object_angle - object_position_angle_from_base_link);

  if (velocity > node_param_.velocity_threshold) {
    if (abs(std::cos(crossing_yaw)) < abs(std::cos(node_param_.angle_threshold))) {
      return true;
    } else {
      return false;
    }
